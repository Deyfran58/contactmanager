...
class DBManager(context: Context) : IDBManager {
    ...
    override fun add(contact: Contact) {
        ...
        val values = ContentValues().apply {
            ...
            // Si Photo es nulo, no la insertamos
            contact.Photo?.let {
    ...
    override fun update(contact: Contact) {
        ...
        val values = ContentValues().apply {
            ...
            put("address", contact.Address)
            put("country", contact.Country)
            // Si Photo es nulo, no la actualizamos
            contact.Photo?.let {
                put("photo", util.convertToByteArray(it))
            }
        }
        ...
        with(cursor) {
            while (moveToNext()) {
                val contact = Contact(
                    ...
                    getBlob(getColumnIndexOrThrow("photo"))?.let { BitmapFactory.decodeByteArray(it, 0, it.size) } // Decodifica el blob a Bitmap
                )
